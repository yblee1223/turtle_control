...
class feedbackCircle : public rclcpp::Node{
    public:
        feedbackCircle() : Node("feedback_circle")
        {
            printf("%s", msg);
            // define msg
            ...
            sub_ = this->create_subscription<turtlesim::msg::Pose> ("/turtle1/pose", 10, std::bind(&feedbackCircle::feedback_callback, this, _1));
            ...
            timer_ = this->create_wall_timer(
                1ms, std::bind(&feedbackCircle::timer_callback, this)
            );
        }
...
int main(int argc, char **argv){
    ...
    auto node = std::make_shared<feedbackCircle>();
